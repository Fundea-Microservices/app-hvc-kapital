/// <reference types="@storybook/angular" />
import { Meta, StoryObj, applicationConfig } from '@storybook/angular';
import { provideHttpClient, withInterceptorsFromDi } from '@angular/common/http';
import { provideAnimations } from '@angular/platform-browser/animations';
import { provideZoneChangeDetection, signal } from '@angular/core';
import { provideRouter } from '@angular/router';
import { provideToastr } from 'ngx-toastr';
import { routes } from '../app/app.routes'; // Ajusta la ruta a tus rutas reales
import DashboardPageComponent from '../app/dashboard/pages/dashboard-page/dashboard-page'; // Ajusta la ruta a tu componente
import { AuthService } from '../services/auth/auth.service';  // Ajusta la ruta a tu servicio

const authServiceMock = {
  getUserStorage: () => ({
    nombre: 'Usuario de Prueba',
    rol: { esAdmin: true }
  }),
  accesos: signal([
    { nombre: 'Transacciones', url: '/dashboard/tesoreria/transacciones', icono: 'cash' },
    { nombre: 'Cuentas', url: '/dashboard/tesoreria/cuentas', icono: 'credit-card' },
    { nombre: 'Bancos', url: '/dashboard/tesoreria/bancos', icono: 'bank' }
  ]),
  accesosLoading: signal(false)
};

const meta: Meta<DashboardPageComponent> = {
  title: 'Pages/Dashboard Page',
  component: DashboardPageComponent,
  decorators: [
    applicationConfig({
      providers: [
        provideZoneChangeDetection({ eventCoalescing: true }),
        provideRouter(routes),
        provideHttpClient(withInterceptorsFromDi()),
        provideAnimations(),
        provideToastr(),
        { provide: AuthService, useValue: authServiceMock }
      ],
    }),
  ],
};

export default meta;
type Story = StoryObj<DashboardPageComponent>;

export const Default: Story = {};