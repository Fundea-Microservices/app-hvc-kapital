# Project Overview: app-hvc-kapital (GrupoCorporativo)
...
ng build
```
Build artifacts will be stored in the `dist/app-hvc-kapital/` directory.
...
- `npm run deploy:ubuntu`: Deploys to `/var/www/hvc-kapital/`.
- `npm run deploy:amazon`: Deploys to `/usr/share/nginx/html/hvc-kapital`.
